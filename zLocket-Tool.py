...
class zLocket:
    def __init__(self, device_token: str="", target_friend_uid: str="", num_threads: int=1, note_target: str=""):
        ...
        if os.name == "nt":
            os.system(
                f"title 💰 {self.NAME_TOOL} {self.VERSION_TOOL} by xyanua. 💰"
         )
    ...
    def _input_(self, prompt_text="", section="config"):
        print(
            f"{xColor.CYAN}┌──({xColor.NEON_GREEN}root@xyanua.{xColor.CYAN})-[{xColor.PURPLE}{section}{xColor.CYAN}]")
    ...
    def _convert_url(self, url: str) -> str:
        ...
        try:
            ...
            if _res.get("status") == 1 and "url" in _res:
                return _res["url"]
            self.messages.append("Lỗi kết nối tới API")
            return ""
        except requests.exceptions.RequestException as e:
            self.messages.append(
                "Lỗi kết nối tới API" + str(e))
            return ""
        except ValueError:
            self.messages.append("Lỗi kết nối tới API")
            return ""
...
def _rand_email_():
    return f"{_rand_str_(15)}@gnolswft"
def _rand_pw_():
